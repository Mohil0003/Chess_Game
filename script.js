  ...
  methods: {
    ...
    capture: function (target) {
      ...
      if (target.name === 'w_king' || target.name === 'b_king') {
        ...
        location.reload(); // This will reload the page after alert is closed
        return;
      }
...
$(document).ready(function() {
  ...
  $('.gamecell').click(function(e) {
    ...
    } else if (main.variables.selectedpiece !='' && target.name != 'null' && selectedpiece.id != target.id && selectedpiece.name.slice(0,1) != target.name.slice(0,1)){ // capture a piece
